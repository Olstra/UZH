...
def place_ship(board, coordinates):
    ...
    if are_valid_ship_coordinates(board, coordinates):
...
def fire(board, coordinates):
    ...
    if are_coordinates_valid(board, coordinates):
        attacked_cell = board[coordinates[0]][coordinates[1]]

        if attacked_cell == SHIP:
            board[coordinates[0]][coordinates[1]] = SUNK_SHIP
            return True
        elif attacked_cell == SUNK_SHIP:
            pass
        else:
...
def are_coordinates_valid(board, coordinates):
    ...
    """
    if coordinates[0] < len(board) and coordinates[1] < len(board):
...
def is_game_over(board):
    ...
    """
    for list in range(BOARD_SIZE-1):
        for index in range(BOARD_SIZE-1):
            if board[list][index] == SHIP:
                return False
