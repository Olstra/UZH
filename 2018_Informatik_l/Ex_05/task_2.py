...
def preview_dognames(list, n):
    ...
    for row in range(n):
        ...
        names.append(current_row['HUNDENAME'])
...
def dognames_count(list):
    ...
    for row in range(len(list)):
        current_name = list[row]['HUNDENAME']
        if current_name not in names:
            nr = 1
            names[current_name] = nr
        else:
...
def top_n_dognames(list, n):
    ...
    names = []
    dict_names = dognames_count(list)
    ranking = sorted(dict_names.items(), key=lambda x: x[1])
    ranking.reverse()

    for i in range(n):
        names.append(ranking[i])
...
def is_valid_row(row, year=None, sex=None):
    ...
    if year:
        if int(row['GEBURTSJAHR_HUND']) != year:
            return False
        if sex:
...
if __name__ == '__main__':
    ...
    print(filter_dognames(dognames, 1998))
